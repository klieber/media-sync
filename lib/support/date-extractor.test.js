...
describe('extractDate', () => {
  beforeAll(() => {
    ...
    jest.spyOn(imageType, 'isJpg');
  });

  describe('from exif', () => {
    beforeAll(() => {
      when(imageType.isJpg).calledWith(FILENAME).mockResolvedValue(true);
    });
  ...
  describe('from file', () => {
    test('birthtime', async () => {
      when(imageType.isJpg).calledWith(FILENAME).mockResolvedValue(false);
