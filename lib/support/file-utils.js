...
import fs from 'fs';
import DropboxContentHasher from '../provider/dropbox/dropbox-content-hasher';
import config from '../config';
import sharp from 'sharp';
import logger from './logger';

const log = logger.create('lib/support/file-utils');
...
async function createFileInfo(sourceFile, targetDirectory) {
  ...
  const dirname = `${targetDirectory.replace(/\/$/, '')}/${formatDate(timestamp, 'yyyy/MM')}`;
  const extension = sourceFile.replace(/^.*\.([^./]*)$/, '$1').toLowerCase();
  const basename = `${formatDate(timestamp, 'yyyy-MM-dd_HH.mm.ss')}_${contentHash.substring(0, 6)}.${extension}`;
  return {
    dirname,
    basename,
    extension,
