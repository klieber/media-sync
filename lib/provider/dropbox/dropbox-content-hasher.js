 ...
 *
 *     import fs from 'fs';
 *     import dch from 'dropbox-content-hasher';
 *
...
class DropboxContentHasher {
  ...
  update(data, inputEncoding) {
    if (!this.#overallHasher) {
      throw new Error('Cannot call "update()" after "digest()" has already been called.');
    }
  ...
  digest(encoding) {
    if (!this.#overallHasher) {
      throw new Error('Cannot call "digest()" after it has already been called.');
    }
