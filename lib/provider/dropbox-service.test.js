...
beforeEach(() => {
  mkdir = jest.spyOn(fs.promises, 'mkdir').mockReturnValue({});
  writeFile = jest.spyOn(fs.promises, 'writeFile').mockReturnValue({});
...
test('DropboxService to download file', async (done) => {
  ...
  expect(mkdir).toHaveBeenCalledWith(TARGET_PATH, { recursive: true });
...
test('DropboxService hash file', async (done) => {
  ...
  when(createReadStream).calledWith(DROPBOX_FILE_NAME).mockReturnValue(stream);
